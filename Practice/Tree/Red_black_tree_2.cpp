...
public:
  ...
  Node *uncle() {
    // if no parent or grandparent, then no unclde
...
class RBTree {
  ...
  void fixRedRed(Node *x) {
    ...
    if(parent->color != BLACK) {
      ...
      if(uncle != NULL && uncle->color == RED) {
        ...
        // Else perform LR, LL, RL, RR cases
        if(parent->isOnLeft()) {
          if(x->isOnLeft()) {
            // for left right
  ...
  // find node tat replaces a deleted node
  ...
  void deleteNode(Node *v) {
    ...
    bool uvBlack = ((u == NULL || u->color == BLACK) &&
                    (v == NULL || v->color == BLACK));
    Node *parent = v->parent;

    if(u == NULL) {
      // u is NULL therefore root null
      if(v == root) {
        root = NULL;
      } else {
        if(uvBlack) {
...
public:
  ...
  void insert(int n) {
    ...
    if(root == NULL) {
      ...
      // conect new node to correct node
      newNode->parent = temp;
